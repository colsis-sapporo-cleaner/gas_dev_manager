const PARAM_BUCKET_NAME = "Bucket";
export class AWS {
//var AWS = (function() {
/*
  // option constants
...
//    init: function AWS(access_key, secret_key) {
     ...
     */
    private request (service, region, action, params, method, payload, headers, uri, options) {
//    request: function(service, region, action, params, method, payload, headers, uri, options) {
      ...
      var Crypto = this.loadCrypto();

      var d = new Date();

      var dateStringFull =  String(d.getUTCFullYear()) + this.addZero(d.getUTCMonth()+1) + this.addZero(d.getUTCDate()) + "T" + this.addZero(d.getUTCHours()) + this.addZero(d.getUTCMinutes()) + this.addZero(d.getUTCSeconds()) + 'Z';
      var dateStringShort = String(d.getUTCFullYear()) + this.addZero(d.getUTCMonth()+1) + this.addZero(d.getUTCDate());
      var payload = payload || '';
      var hashedPayload = Crypto.SHA256(payload);
      var method = method || "GET";
      var uri = uri || "/";
      var host = this.getHost(service, region, bucket);
      var headers = headers || {};
      var request;
      var query;
      if(method.toLowerCase() == "post") {
        ...
        query = "Action="+action;
        if(params) {
          Object.keys(params).sort(function(a,b) { return a<b?-1:1; }).forEach((name) => {
            query += "&"+name+"="+this.fixedEncodeURIComponent(params[name]);
          });
        }
        request = "https://"+host+uri+"?"+query;
      }

      var canonQuery = this.getCanonQuery(query);
      var canonHeaders = "";
      var signedHeaders = "";
      headers["Host"] = host;
      ...
      var key = this.getSignatureKey(Crypto, this.secretKey, dateStringShort, region, service);
      var signature = Crypto.HMAC(Crypto.SHA256, StringToSign, key, { asBytes: false });

      var authHeader = algorithm +" Credential="+this.accessKey+"/"+scope+", SignedHeaders="+signedHeaders+", Signature="+signature;
     ...
     */
     private setNewKey (access_key, secret_key) {
